#!/usr/bin/env bash
# crux-trainer.sh — Interactive CRUX Linux Installation Trainer
# Follows the official CRUX 3.8 Handbook step by step.
#
# Usage: bash crux-trainer.sh
#
# https://github.com/davidleath2005/crux-trainer
...
# shellcheck source=modules/04_setup.sh
source "${SCRIPT_DIR}/modules/04_setup.sh"
# shellcheck source=modules/05_chroot.sh
source "${SCRIPT_DIR}/modules/05_chroot.sh"
# shellcheck source=modules/06_ports.sh
source "${SCRIPT_DIR}/modules/06_ports.sh"
...
# shellcheck source=modules/14_post_install.sh
source "${SCRIPT_DIR}/modules/14_post_install.sh"
# shellcheck source=modules/15_finalize.sh
source "${SCRIPT_DIR}/modules/15_finalize.sh"
...
MODULES=(
    ...
    "module_04_setup:04 — CRUX Setup Script"
    "module_05_chroot:05 — Chroot Setup"
    "module_06_ports:06 — Ports & prt-get"
    ...
    "module_12_bootloader:12 — Bootloader"
    "module_13_users:13 — Users & Passwords"
    "module_14_post_install:14 — Post-Installation"
    "module_15_finalize:15 — Finalize & Reboot"
)
    ...
    if [[ -f "$PROGRESS_FILE" ]]; then
        ...
        case "$MENU_CHOICE" in
            1)
                ...
                local idx=0
                for entry in "${MODULES[@]}"; do
                    # Map module index to first step ID
                    local first_step
        ...
        show_menu "Welcome to CRUX Trainer! What would you like to do?" \
