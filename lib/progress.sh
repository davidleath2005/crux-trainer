#!/usr/bin/env bash
# lib/progress.sh — Progress tracking for CRUX Trainer

PROGRESS_FILE="${PROGRESS_FILE:-/tmp/crux-trainer-progress}"
...
verify_system_state() {
    ...
    if is_step_done "03.4"; then
        if ! mountpoint -q /mnt 2>/dev/null; then
            log_warn "Step 03.4 (mount) is marked done but /mnt is not mounted."
            (( issues++ ))
        fi
    fi
    ...
    if is_step_done "04.3"; then
        if [[ ! -d /mnt/etc ]]; then
            log_warn "Step 04.3 (setup) is marked done but /mnt/etc does not exist."
            (( issues++ ))
        fi
    fi
