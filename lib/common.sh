#!/usr/bin/env bash
# lib/common.sh — Shared utility functions for CRUX Trainer
...
check_root() {
    if [[ $EUID -ne 0 ]]; then
        ...
        log_info  "Try: sudo bash crux-trainer.sh"
        exit 1
    fi
}
...
# ─── require_live ────────────────────────────────────────────────────────────
require_live() {
    if [[ ! -f /etc/crux-release ]] && [[ ! -f /run/livecd ]]; then
        log_warn "This module is designed to run from the CRUX live environment."
...
require_chroot() {
    ...
    if [[ "$root_dev" == "$outside_dev" ]]; then
        ...
        log_info  "Run: chroot /mnt /bin/bash"
        exit 1
    fi
}
